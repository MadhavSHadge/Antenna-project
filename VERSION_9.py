...
class ArduinoMotorControl(QMainWindow):
    ...
    def write_encoder(self):
        try:
            ...
            self.ser.write(f'P{pulses},{speed}'.encode())
            self.write_motor_settings()
            QMessageBox.information(self, "Pulse Count Set", f"Motor will run for {pulses} pulses at speed {speed}")
        except ValueError:
            QMessageBox.critical(self, "Error", "Please enter valid integer values for pulse count and speed")
    ...
    def start_timer(self):
        ...
        pulses_per_second = rotations_per_second * pulses_per_rotation
        required_speed = pulses_per_second / total_time

        # Convert speed to PWM (0-255)
        pwm_value = min(max(int((required_speed / pulses_per_second) * 255), 0), 255)
        self.speed_entry.setText(str(pwm_value))
        self.set_speed()
