...
# Function to log speed and pulse values with date and time
...
def set_speed():
    try:
        ...
        if 0 <= speed <= 255:
            ...
            log_values(speed=speed)  # Log speed value
        else:
...
def write_encoder():
    try:
        ...
        log_values(speed=speed, pulses=pulses)  # Log pulse and speed values
    except ValueError:
...
# Clock label
clock_label = tk.Label(root, font=('times new roman', 15, 'bold'))
clock_label.grid(row=2, column=0, columnspan=2, sticky='e')
